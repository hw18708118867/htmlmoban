			...
			foreach($file_list as $_k=>$_v) {	
				if($_v[fileurl]){
					...
					if($downloadlink) {
						...
						$list_str[] = "<a class='down_link' href='".APP_PATH."index.php?m=content&c=down&a_k={$a_k}' title='下载{$this->data['title']}模板' target='_blank'>进入模板下载地址</a>";
					} else {
						$list_str[] = "<a class='down_link' href='{$_v[fileurl]}' title='下载{$this->data['title']}模板' target='_blank'>进入模板下载地址</a>";
					}
				}
			}
		}
