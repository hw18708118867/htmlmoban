...
/* TMSlider 0.4.1 */
;(function(jQuery){
	jQuery.fn.TMSlider=jQuery.fn.TMS=jQuery.fn._TMS=function(o){
		return this.each(function(){
					...
					anims: {			
						centralExpand:function(el,last){				
							jQuery(el).each(function(){
								var th=jQuery(this).css({visibility:'hidden'}),
						...
						fadeThree:function(el,last){
							...
							jQuery(el).each(function(i){
								var th=jQuery(this).show().css({left:-_.width/4,top:0,zIndex:2,opacity:0}),
						...
						zoomer:function(el,last){
							...
							el.each(function(){
								...
								,buff=jQuery(new Image())
								,canvas=ie?jQuery(new Image()):jQuery('<canvas></canvas>')
								,width,height
								,duration=_.duration
								...
								,calcCSS=function(width,height,k){
									...
									swtch(motion,{
										...
										,move:function(){
											css=jQuery.extend(true,css,side!='center'?{start:{width:width*k,height:height*k}}:{})
											
											css=jQuery.extend(true,css,swtch(side,{
												left:{
													finish:{						
														left:0
														,top:-(height*k-height)
						...
						changeFu:function(n){
							...
							var _fu=function(){
								...
								if(jQuery.browser.msie&&jQuery.browser.version<9&&_.preset=='zoomer')
									_.preset='simpleFade'
									,_.duration=1000
								...
								if(_.preset_!=_.preset)
									 _.du=_.duration,
									 _.ea=_.easing,
									jQuery.extend(_,_.presets[_.preset]),
									 _.duration=_.du,
									 _.easing=_.ea,
									_.preset_=_.preset								
								if(_.preset=='random')									
									jQuery.extend(_,_.presets[_.prsts[parseInt(Math.random()*_.prsts.length)]])
						...
						showFu:function(){
							...
							_.int=setInterval(function(){
								if(way.length)
									...
									clearInterval(_.int)//,
									//jQuery.when(_.maskC).then(function(){_.maskC.remove(),delete _.maskC})
								},_.interval)
							_.bl=true
						},
						...
						afterShow:function(){			
							...
							if(_.banner)
								jQuery.when(jQuery('.'+_.bannerCl,_.me)).then(function(){
									jQuery('.'+_.bannerCl,_.me).not(_.banner).remove()
								}),
						...
						paginationFu:function(){
							...
							if(_.pags.children().length==0)
								jQuery(_.itms).each(function(n){
									var li=jQuery('<li></li>').data({num:n})
									_.pags.append(li.append('<a href="#"></a>'))
								})
							else
								_.pags.find('li').each(function(n){
									jQuery(this).data({num:n})
								})
							if(_.pagNums)
							...
							_.pags.delegate('li>a',_.changeEv,function(){
								_.changeFu(jQuery(this).parent().data('num'))
								return false
							})
							_.pags.addClass(_.paginationCl)
							_.pags=jQuery('li',_.pags)			
						...
						bannersFu:function(){
							...
							jQuery(_.items,_.me).each(function(i){
								var tmp
								_.bnnrs[i]=(tmp=jQuery('.'+_.bannerCl,this)).length?tmp.css({zIndex:999}):false
							})
			...
			typeof o=='object'&&jQuery.extend(_,o)
			_.me||_.init(_.me=th)
		})		
	}
})(jQuery)
